...
class JudgeNode:
    ...
    def __init__(self):
        ...
        self.cmdstartPub_ = rospy.Publisher('/m3e/cmd_start', Bool, queue_size=100)

        self.commandSub_ = rospy.Subscriber('/m3e/cmd_string', String, self.commandCallback)
        self.ringSub_=rospy.Subscriber('/m3e/ring',String,self.ringCallback)
        self.poseSub_ = rospy.Subscriber('/gazebo/model_states', ModelStates, self.poseCallback)
        self.strSub_ = rospy.Subscriber('/m3e/target_result', String, self.strCallback)
    ...
    def echoMessage(self):
        if self.node_state_ == self.NodeState.WAITING:
            ...
            print('')
            print('已穿过圆环数目:'+str(self.ring_num_))
            print('')
            ...
            print('当前总得分：'+ str(self.score_)+'  请裁判核实并记录')
            pass
    ...
    def printCompareResult(self):
        ...
        if self.target_result_ is None:
            ...
            print('正确识别目标数:       ' + str(self.correct_results))
    ...
    def poseCallback(self, msg):
        if 'iris' in msg.name:
            ...
            if pose.position.z < 0.2 and self.node_state_ == self.NodeState.LANDING:
        ...
        if not self.is_result_received_:
            ...
            if len(self.target_result_) == 2:
                for i in range(2):
                    if self.target_groundtruth_[i] == self.target_result_[i]:
                        self.correct_results += 1
            self.score_=self.score_+self.correct_results*15
        pass
